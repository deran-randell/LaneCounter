...
export default function LaneDetail(props) {
  ...
  const DetailTimePeriods = [
    ...
    { label: "Last 12 hours", value: 1, minutes: 12 * 60, update_rate: 60, aggregation: "per minute" },
    { label: "Last 6 hours", value: 2, minutes: 6 * 60, update_rate: 60, aggregation: "per minute" },
    { label: "Last 3 hours", value: 3, minutes: 3 * 60, update_rate: 60, aggregation: "per minute" },
    { label: "Last 1 hour", value: 4, minutes: 60, update_rate: 60, aggregation: "per minute" },
    { label: "Last 30 minutes", value: 5, minutes: 30, update_rate: 10, aggregation: "per minute" },
    { label: "Last 10 minutes", value: 6, minutes: 10, update_rate: 10, aggregation: "per minute" },
    { label: "Last 1 minute", value: 7, minutes: 1, update_rate: 1, aggregation: "per minute" },
  ];
  ...
  const columns = [
    // { name: "Lane", selector: (row) => row.lane_id, sortable: true },
    { name: "Device ID", selector: (row) => row.device_id, sortable: true },
    { name: "Sensor Number", selector: (row) => row.sensor_number, sortable: true },
    { name: "Sensor State", selector: (row) => row.sensor_state, sortable: true },
    { name: "Timestamp", selector: (row) => row.unix_timestamp, sortable: true },
  ];
  ...
  const [tableDataHistory, setTableDataHistory] = useState(null);
  ...
  function decodeSensorState(sensor_state) {
    switch (sensor_state) {
      case "1":
        sensor_state = "manual_calibration";
        break;
      case "2":
        sensor_state = "autocalibration_triggered";
        break;
      case "3":
        sensor_state = "cleaning_started";
        break;
      case "4":
        sensor_state = "cleaning_complete";
        break;
  ...
  function decodeSensorTimestamp(sensor_timestamp) {
    sensor_timestamp = new Date(sensor_timestamp * 1000).toISOString();
  ...
  function SensorStateColor(state) {
    switch (state) {
      default:
        break;
      case "1": // manual_calibration
        return (state = "purple");

      case "2": // autocalibration_triggered
        return (state = "orange");

      case "3": // cleaning_started
        return (state = "green");

      case "4": // cleaning_complete
        return (state = "blue");
    }
  }
  ...
  useQuery(fetch_sensor_states_today_query_name, () => {
    ...
    axios.get(url).then((res) => {
      ...
      response.map((state) => {
        state.sensor_state = decodeSensorState(state.sensor_state);
        state.unix_timestamp = decodeSensorTimestamp(state.unix_timestamp);
        return state;
      });
      setTableData(JSON.parse(formatResponse(response)));
    });
  });
  ...
  useQuery(fetch_sensor_states_history_query_name, () => {
    ...
    axios.get(url).then((res) => {
      ...
      setTableDataHistory(JSON.parse(formatResponse(response)));
    });
  });
  ...
  return (
    <>
      <div>
        ...
        {showLog && sensorDidCalibrate.length > 0 && <DataTable title="Device Status Log" columns={columns} data={tableData} pagination />}
        ...
        {showHistoryLog && sensorDidCalibrateHistory.length > 0 && <DataTable title="Device Status Log" columns={columns} data={tableDataHistory} pagination />}

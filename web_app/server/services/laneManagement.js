...
const state = {
  none: 0,
  manual_calibration: 1,
  autocalibration_triggered: 2,
  cleaning_started: 3,
  cleaning_complete: 4,
};
...
function checkState(sensor_states) {
  ...
  for (let i = 0; i < sensor_states.length; i++) {
    if (sensor_states[i] == state.manual_calibration || sensor_states[i] == state.autocalibration_triggered || sensor_states[i] == state.cleaning_started || sensor_states[i] == state.cleaning_complete) {
      sensor_number = i;
    }
  }

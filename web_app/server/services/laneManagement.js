...
var mothLanes = [
    new MothLane(1, m), new MothLane(2, m), new MothLane(3, m), new MothLane(4, m), new MothLane(5, m), new MothLane(6, m), new MothLane(7, m), 
    new MothLane(8, m), new MothLane(9, m), new MothLane(10, m), new MothLane(11, m), new MothLane(12, m), new MothLane(13, m), new MothLane(14, m), 
    new MothLane(15, m), new MothLane(16, m), new MothLane(17, m), new MothLane(18, m), new MothLane(19, m), new MothLane(20, m), new MothLane(21, m),
    new MothLane(22, m)];
    ...
    if (!initialised) {
        ...
        for (let i = 0; i < 3; i++)
        {
            ...
            console.log("Lane " + (i+1) + 
                ": total=" + mothLanes[i].total + 
                ", count=" + mothLanes[i].count +
...
async function getLaneTotal(lane_id) {
    try {
        const rows = await db.query(
            "SELECT CurrentLaneTotal(?) AS Total",
            [lane_id]    
        );
...
async function getLastLaneData(lane_id) {
    try {
        const rows = await db.query(
            "SELECT moth_count, millis, UNIX_TIMESTAMP(time_stamp) AS unix_timestamp FROM LaneCount WHERE lane_id = ? ORDER BY time_stamp DESC LIMIT 1",
            [lane_id]    
        );
...
async function getCurrentLaneDevice(lane_id) {
    try {
        const rows = await db.query(
            "SELECT CurrentLaneDevice(?) AS Device",
            [lane_id]    
        );
    ...
    try {
        if (initialised) {
            ...
            // Not first time in, do others
                ...
                else {  // System has processed at least 1 message for this lane
...
function addCountForLane(lane_id, count, delta, millis) {
    try {
        // Initialisation check has been done in calling function
        const i = lane_id-1;        
...
async function addLaneDevice(laneId, deviceId) {
    try {
      ...
      const result = await db.query("INSERT INTO lanedevice (lane_id, device_id, time_stamp) VALUES (?, ?, FROM_UNIXTIME(?))",
      [laneId, deviceId, time_stamp]);

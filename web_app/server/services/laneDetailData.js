...
async function getLaneDataWithPagination(laneId, page = 1) {
  try {
    ...
    const rows = await db.query("SELECT * FROM lanecount WHERE lane_id = ? LIMIT ?,?", [laneId, offset, config.listPerPage]);
  ...
  try {
    ...
    if (aggregate_seconds !== 0) {
      sql_string =
        ...
        "FLOOR(UNIX_TIMESTAMP(time_stamp)/(" +
        aggregate_seconds +
        ")) AS timekey " +
        "FROM LaneCount " +
        "WHERE lane_id = ? " +
      ...
      sql_string = "SELECT UNIX_TIMESTAMP(time_stamp) AS unix_timestamp, moth_delta FROM lanecount " + "WHERE lane_id = ? " + time_str + "ORDER BY time_stamp ASC";
    }
...
async function addLaneData(laneId, count, delta, millis, time_stamp, sensor_states) {
  ...
  try {
    const result = await db.query("INSERT INTO lanecount (lane_id, moth_count, moth_delta, millis, time_stamp, sensor_states) VALUES (?, ?, ?, ?, FROM_UNIXTIME(?), ?)", [laneId, count, delta, millis, time_stamp, sensor_states]);
